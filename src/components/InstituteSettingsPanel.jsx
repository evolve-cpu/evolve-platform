...
// The settings "page" content — deliberately NOT a page of its own. It's
// rendered inside InstitutePublicPage's existing shell (same topbar, same
// sidebar) so switching into settings just swaps the main content area
// instead of unmounting/remounting the whole app shell the way a separate
// route did. See InstitutePublicPage's `tab === "settings"` branch.
//
...
export default function InstituteSettingsPanel({ org, members, user, slug, onBack, onNavigateTab, reloadOrg, reloadMembers }) {
  const [deletedByName, setDeletedByName] = useState(null);
  const [toast, setToast] = useState(null);

  const [visibilityDraft, setVisibilityDraft] = useState(org.visibility || "public");
  ...
  // re-seed local drafts whenever the org row underneath us changes (e.g.
  // after a save round-trips through the parent's reload)
  useEffect(() => {
  ...
  }, [org.id, org.visibility, org.notification_prefs]);
  ...
  useEffect(() => {
    ...
    supabase
      .from("profiles")
      .select("name")
      .eq("id", org.deleted_by)
      .maybeSingle()
      .then(({ data }) => {
        if (!cancelled) setDeletedByName(data?.name || null);
      });
    return () => {
      cancelled = true;
    };
  ...
  return (
    <div className="max-w-[860px] w-full mx-auto flex flex-col gap-5">
      <div>
        <button onClick={onBack} className="inline-flex items-center gap-1 text-evolve-lavender-indigo hover:underline text-xs font-bold mb-3">
        ...
        <>
          <div className="rounded-2xl border border-evolve-red/30 p-5 sm:p-6" style={{ background: "linear-gradient(180deg, rgba(223,5,134,.08), #1c1c1e)" }}>
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-white/50 text-xs mt-1 leading-relaxed">
                  the public page is offline and team access is disabled. restore until <strong className="text-white">{fmtDate(permanentAtMs)}</strong> — after that, deletion is permanent.
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <a href="mailto:contact@evolvedesign.academy" className="text-center border border-white/15 text-white/60 hover:text-white font-bold text-[13px] rounded-xl py-3 transition-colors">
                contact support
              </a>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            ...
            <p className="text-white/45 text-xs leading-relaxed">
              the public page shows "not found" to visitors, and team members can't sign in to this space. restoring before <strong className="text-white">{fmtDate(permanentAtMs)}</strong> brings
        ...
        <>
          ...
          >
            <div className="flex flex-col items-center text-center gap-2.5 py-8 px-5 rounded-xl border border-dashed border-white/15 bg-white/[0.02]">
              ...
              <button onClick={() => onNavigateTab("programs")} className="mt-1 text-xs font-bold text-evolve-lavender-indigo">
