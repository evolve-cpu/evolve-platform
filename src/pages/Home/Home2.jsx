...
const Home = ({
  ...
  useLayoutEffect(() => {
    ...
    const rafId = requestAnimationFrame(() => {
      ...
      // Prevent native scroll during swipe inside the pin zone so window.scrollY
      // stays where goToSection put it and doesn't fight our section tracking.
      const handleTouchMove = (e) => {
        if (window.scrollY < (stRef.current?.end ?? 0)) e.preventDefault();
      };
      const handleTouchEnd = (e) => {
        if (isAnimating || window.scrollY >= (stRef.current?.end ?? 0)) return;
        const dy = touchStartY - e.changedTouches[0].clientY;
        if (Math.abs(dy) < 20) return; // lower threshold — easier to trigger per card
