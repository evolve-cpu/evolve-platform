...
// import Scene1_4, { useScene1_4Timeline } from "./Scene1_4";
// import GrainTexture from "../../components/GrainTexture";
// import { logo_main } from "../../assets/images/Home";
...
//       !scene1Refs.current.container ||
//       !sceneNewRefs.current.container || // NEW SCENE CHECK
//       !scene1Refs.current.logoWrapper || // ← ADD THIS
//       !scene1_1Refs.current.container ||
//       !scene1_2Refs.current.container ||
//       // scene1_3Refs.current.container REMOVED
//       !scene1_4Refs.current.container
...
//       // Get timelines for each scene
//       // const tlNew = useSceneNewTimeline(sceneNewRefs.current, isMobile); // NEW SCENE TIMELINE
//       const sceneNewRefsWithLogo = {
//         ...sceneNewRefs.current,
//         logoWrapper: scene1Refs.current.logoWrapper // logo from Scene1
//       };
//       const tlNew = useSceneNewTimeline(sceneNewRefsWithLogo, isMobile);
//       const tl2 = useScene1_2Timeline(scene1_2Refs.current, isMobile);
//       const tl3 = useScene1_1Timeline(scene1_1Refs.current, isMobile); // Scene1_1 is now 3rd
//       const tl4 = useScene1_4Timeline(scene1_4Refs.current, isMobile);
...
//         <div
//           ref={(el) => {
//             // Sync this ref back to scene1Refs so timeline can find it
//             if (scene1Refs.current) scene1Refs.current.logoWrapper = el;
//           }}
//           className="absolute inset-0 z-[20] flex items-center justify-center pointer-events-none"
//           style={{
//             willChange: "transform, opacity",
//             opacity: 0 // GSAP controls opacity via scene1 timeline
//           }}
//         >
//           <div style={{ width: isMobile ? "28vw" : "10vw", height: "auto" }}>
//             <img src={logo_main} /> // import LogoMainSVG from Scene1 or define
//             inline here
//           </div>
//         </div>
//       )}
...
const Home = ({
  ...
  // Ref for the Home-level logo unit (visible only during SceneNew)
  const logoUnitRef = useRef(null);
  ...
  const handleLogoClick = React.useCallback(() => {
    ...
    const scrollTrigger = masterTimelineRef.current?.scrollTrigger;
    if (!scrollTrigger || !snapPointsRef.current?.length >= 2) {
      window.scrollTo({ top: 0, behavior: "smooth" });
      return;
    }
    const sceneNewFirstStepProgress = snapPointsRef.current[1] || 0.05;
  ...
  useLayoutEffect(() => {
    ...
    if (
      ...
      !scene1_2Refs.current.container ||
      !scene1_4Refs.current.container ||
      !logoUnitRef.current // ← wait for logoUnit div
    ) {
    ...
    const id = requestAnimationFrame(() => {
      ...
      // logoUnit starts hidden — shown only when SceneNew is active
      gsap.set(logoUnitRef.current, { opacity: 0 });

      // Pass logoUnit to SceneNew timeline so badge can bloom around it
      const sceneNewRefsWithLogo = {
        ...sceneNewRefs.current,
        logoUnit: logoUnitRef.current
      };

      const tlNew = useSceneNewTimeline(sceneNewRefsWithLogo, isMobile);
      const tl2 = useScene1_2Timeline(scene1_2Refs.current, isMobile);
      const tl3 = useScene1_1Timeline(scene1_1Refs.current, isMobile);
      const tl4 = useScene1_4Timeline(scene1_4Refs.current, isMobile);
      ...
      // CROSSFADE — no zoom, no scale. Elements fade in/out smoothly.
      // Simultaneously:
      //   • SceneNew fades in (opacity 0→1)
      //   • Scene1 fades out (opacity 1→0)
      //   • logoUnit fades in (opacity 0→1) — it's above both scenes
      master
        .to(sceneNewRefs.current.container, {
          opacity: 1,
          duration: 0.6,
        ...
        // logoUnit fades in at the same time — it appears centered above SceneNew
        .to(
          logoUnitRef.current,
          {
            opacity: 1,
      ...
      // ── When leaving SceneNew → fade logoUnit out ─────────────────────
      // This ensures logo doesn't persist into Scene1_2 and beyond
      master.to(logoUnitRef.current, {
        opacity: 0,
        duration: 0.4,
        ease: "power2.in"
      });

      // ── TRANSITION 2: SceneNew → Scene1_2 ────────────────────────────────
      master
        .to(sceneNewRefs.current.container, {
          y: "100%",
          duration: 1.2,
        ...
        .to(
          scene1_2Refs.current.container,
          { y: "0%", duration: 1.2, ease: "power2.inOut", force3D: true },
          "<"
        );

      if (tl2) master.add(tl2);

      // ── TRANSITION 3: Scene1_2 → Scene1_1 ────────────────────────────────
      master
        .to(scene1_2Refs.current.container, {
          x: "-100%",
          duration: 1.2,
        ...
        .to(
          scene1_1Refs.current.container,
          { x: "0%", duration: 1.2, ease: "power2.inOut", force3D: true },
          "<"
        );

      if (tl3) master.add(tl3);

      // ── TRANSITION 4: Scene1_1 → Scene1_4 ────────────────────────────────
      master
        .to(scene1_4Refs.current.container, {
          opacity: 1,
          duration: 0.3,
      ...
      const addStepsFromTimeline = (tl, labels) => {
        ...
        labels.forEach((label) => {
          const localTime = tl.labels?.[label];
          if (typeof localTime !== "number") return;
          stepProgresses.push((start + localTime) / masterDur);
        });
      };
    ...
    return () => {
      ...
      try {
        if (masterTimelineRef.current) {
          masterTimelineRef.current.scrollTrigger?.kill();
          masterTimelineRef.current.kill();
          masterTimelineRef.current = null;
        }
  ...
  const logoImgSize = isMobile ? "30vw" : "12vw";
  // Must match Scene1's sharedWrapper y offset so logo appears in same position
  const logoYOffset = isMobile ? "-16vh" : "-20vh";
  ...
  return (
    ...
    >
      ...
      {/* ── Home-level logo unit ─────────────────────────────────────────────── */}
      {/* Lives ABOVE all scenes (z-[20]).                                       */}
      {/* ONLY visible during SceneNew — master timeline fades it in and out.    */}
      {/* Starts at opacity:0, GSAP brings it to opacity:1 in Transition 1,     */}
      {/* then fades it back to opacity:0 before Transition 2.                  */}
      {/* This prevents it from appearing sticky across other scenes.           */}
      {introDone && (
        <div
          ref={logoUnitRef}
          className="absolute inset-0 z-[20] flex items-center justify-center pointer-events-none"
          style={{
            willChange: "transform, opacity",
            opacity: 0 // GSAP controls this — starts hidden
          }}
        >
          <img
            src={logo_main}
            ...
            style={{
              ...
              height: "auto",
              display: "block",
              // Positioned at the same Y as Scene1 left it
