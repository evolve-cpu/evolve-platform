...
export default function AnantStudentProfile() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const { data: rev } = await supabase
        ...
        .select(
          "id, created_at, review_status, review_report_url, portfolio_link, portfolio_file_url"
        )
        .eq("email", email)
  ...
  const statusMeta = {
    pending: {
      ...
      bord: "rgba(251,191,36,0.2)"
    },
    in_review: {
      label: "review in progress",
      color: "#60a5fa",
      bg: "rgba(96,165,250,0.1)",
      bord: "rgba(96,165,250,0.2)"
    },
    done: {
      label: "report ready",
      color: "#4ade80",
      bg: "rgba(34,197,94,0.1)",
      bord: "rgba(34,197,94,0.2)"
    },
    rejected: {
      label: "not accepted",
      color: "#f87171",
      bg: "rgba(248,113,113,0.1)",
      bord: "rgba(248,113,113,0.2)"
    }
  };
  ...
  return (
    ...
    >
      {/* nav */}
      <header
        className="flex items-center justify-between px-5 md:px-8 border-b"
        style={{
          height: "64px",
          background: "#060c17",
          borderColor: "#0d1f3c"
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full px-6 py-10 flex flex-col gap-6">
        ...
        {/* profile card */}
        {/* <div
          className="rounded-2xl border p-6 grid grid-cols-2 md:grid-cols-3 gap-5"
          style={{ borderColor: border, background: cardBg }}
        > */}
        {/* profile card */}
        <div
          className="rounded-2xl border p-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5"
          style={{ borderColor: border, background: cardBg }}
        >
          {[
            ...
            {
              label: "year",
              value: student.year ? `Year ${student.year}` : "—"
            }
          ].map(({ label, value }) => (
            <div key={label}>
              <p
            ...
            >
              <div>
                ...
                <p className="text-sm mt-0.5" style={{ color: sub }}>
                  submit your portfolio for an expert review — feedback within
                  5–7 working days.
                </p>
              </div>
              <button
                    ...
                    {[
                      ...
                      },
                      {
                        ...
                        value: review.portfolio_link ? (
                          ...
                          <a
                            href={review.portfolio_file_url}
                            target="_blank"
                            rel="noreferrer"
                            className="underline underline-offset-2"
                            style={{ color: "#60a5fa" }}
      ...
      <footer
        className="px-6 py-4 text-xs border-t flex justify-between"
